...
export default function HomePage() {
  ...
  const generateMarkdownReport = async (accountName: string): Promise<string | null> => {
    try {
      const response = await axios.post<{ response: string }>(
        'http://localhost:8000/generate_report',
        JSON.stringify({ company_name: accountName }), 
        {
          headers: {
...
*Generated by SELLY • More insights. More sales.*`
  ...
  const handleGenerate = async () => {
    ...
    const markdownReport = await generateMarkdownReport(accountName);
    setBriefData({
      accountName,
      includeClosedWon,
      markdownReport : markdownReport || "Error generating report. Please try again.",
    })
